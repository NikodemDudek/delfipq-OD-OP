...
def setup_multiarc_estimation(propagator_settings, observation_settings, bodies, arcs_start_epochs, estimate_constant_drag_coefficient, estimate_arcwise_drag_coefficient, drag_arcs_start_epochs):
...
def process_results_singlearc_prediction_comparison(sgp4_cartesian_states_estimation, sgp4_cartesian_states_prediction, propagation_cartesian_states_estimation, propagation_cartesian_states_prediction, conventional_cartesian_states):

    # Calculate final residuals
    cartesian_final_residuals_estimation = sgp4_cartesian_states_estimation[:,:3] - propagation_cartesian_states_estimation[:,:3]
    cartesian_final_residuals_prediction = sgp4_cartesian_states_prediction[:,:3] - propagation_cartesian_states_prediction[:-1,:3]
...
def plot_residuals_estimation(tnw_final_residuals, observation_epochs, tles_batch_epochs, show_tles_epochs):
    ...
    plt.scatter((observation_epochs - observation_epochs[0])/3600, tnw_final_residuals[:, 0]/1000, color="blue", marker='.', label="t-residual")  
    plt.scatter((observation_epochs - observation_epochs[1])/3600, tnw_final_residuals[:, 1]/1000, color="green", marker='.', label="n-residual")
    plt.scatter((observation_epochs - observation_epochs[2])/3600, tnw_final_residuals[:, 2]/1000, color="red", marker='.', label="w-residual")

    if show_tles_epochs:
        for i, tle_epoch in enumerate((tles_batch_epochs - observation_epochs[0])/3600):
            if i == 1:
...
def plot_residuals_singlearc_prediction_comparison(tnw_final_residuals, tnw_final_residuals_conventional, conventional_epochs, observation_epochs, tles_batch_epochs, prediction_start_epoch, show_tles_epochs):
...
def plot_accelerations(dependent_variables, propagation_epochs):
    ...
    plt.plot((propagation_epochs - propagation_epochs[0])/3600, acceleration_norm_pm_sun, label='PM Sun')

    acceleration_norm_pm_moon = dependent_variables[:,2]
    plt.plot((propagation_epochs - propagation_epochs[0])/3600, acceleration_norm_pm_moon, label='PM Moon')

    acceleration_norm_pm_mars = dependent_variables[:,3]
    plt.plot((propagation_epochs - propagation_epochs[0])/3600, acceleration_norm_pm_mars, label='PM Mars')

    acceleration_norm_pm_venus = dependent_variables[:,4]
    plt.plot((propagation_epochs - propagation_epochs[0])/3600, acceleration_norm_pm_venus, label='PM Venus')

    acceleration_norm_pm_jupiter = dependent_variables[:,5]
    plt.plot((propagation_epochs - propagation_epochs[0])/3600, acceleration_norm_pm_jupiter, label='PM Jupiter')

    acceleration_norm_sh_earth = dependent_variables[:,6]
    plt.plot((propagation_epochs - propagation_epochs[0])/3600, acceleration_norm_sh_earth, label='SH Earth')

    acceleration_norm_aero_earth = dependent_variables[:,7]
    plt.plot((propagation_epochs - propagation_epochs[0])/3600, acceleration_norm_aero_earth, label='Aerodynamic Earth')

    acceleration_norm_rp_sun = dependent_variables[:,8]
    plt.plot((propagation_epochs - propagation_epochs[0])/3600, acceleration_norm_rp_sun, label='Radiation Pressure Sun')
